...
If you haven't adopted or can not map your `administrationRoute`, `dosageForm`, or `container` values to HL7, you can use your own codes. To make sure the data user understands these codes, you should name the code lists used in your publication policy document, and provide access to tables where the codes can be looked up.
However, as the dosage forms and pack sizes tend to differ significantly across countries, it is strongly recommended to use standardized codes so the unit prices are easier to compare.
## Background
